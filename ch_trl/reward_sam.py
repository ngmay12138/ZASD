...
class EnhancedRewardSystem:
    ...
    def _init_llm_judge(self):
        ...
        # 获取 "0" 和 "1" 的 token id，用于 logprob 连续评分
        self.token_id_0 = self.tokenizer.encode("0", add_special_tokens=False)[0]
        self.token_id_1 = self.tokenizer.encode("1", add_special_tokens=False)[0]
        ...
        print(f"Token IDs: '0'={self.token_id_0}, '1'={self.token_id_1}")
    ...
    def _llm_evaluate(self, prompts: List[str]) -> List[float]:
        if not prompts:
            return []
        outputs = self.llm.generate(prompts, self.sampling_params)
        scores = []
        for output in outputs:
            if output and output.outputs and output.outputs[0].logprobs:
                # 取第一个生成 token 的 logprob 分布
                first_token_logprobs = output.outputs[0].logprobs[0]
                # 提取 "0" 和 "1" 的 logprob（找不到给极小值）
                logp_0 = -100.0
                logp_1 = -100.0
                for token_id, logprob_obj in first_token_logprobs.items():
                    if token_id == self.token_id_0:
                        logp_0 = logprob_obj.logprob
                    elif token_id == self.token_id_1:
                        logp_1 = logprob_obj.logprob
                # softmax 得到连续 [0,1] 分数
                max_logp = max(logp_0, logp_1)
                exp_0 = math.exp(logp_0 - max_logp)
                exp_1 = math.exp(logp_1 - max_logp)
                score = exp_1 / (exp_0 + exp_1)
            else:
                # fallback: 解析文本响应
    ...
    @staticmethod
    def shaped_iou_reward(iou: float) -> float:
        """基于实际IoU数据分布的非线性变换

        数据分布特征（来自iou_records.json, 1334条）：
        - 71.2% IoU=0（SAM3分割失败）
        - 非零IoU集中在0.6~0.9（占非零的82.5%，中位数0.783）
        - 0.1~0.5区间样本极少

        设计思路：
        - IoU < 0.05: 噪声级别，直接归零
        - IoU 0.05~0.3: 低质量分割，压缩到 [0.05, 0.15]
        - IoU 0.3~0.6: 中等质量，压缩到 [0.15, 0.30]
        - IoU 0.6~0.9: 密集区，展开到 [0.30, 1.0]（占70%的reward空间）
        - IoU > 0.9: 封顶 1.0

        效果：密集区reward range从0.297提升到0.692，GRPO组内方差提升约39%
        """
        if iou <= 0.05:
            return 0.0
        elif iou <= 0.3:
            return 0.05 + 0.10 * (iou - 0.05) / 0.25
        elif iou <= 0.6:
            return 0.15 + 0.15 * (iou - 0.3) / 0.3
        elif iou <= 0.9:
            return 0.30 + 0.70 * (iou - 0.6) / 0.3
        else:
            return 1.0

    def compute_description_reward_sam3(self, original_image_path: str, description: str, mask_path: str) -> float:
        """使用SAM3计算描述奖励（IoU + 非线性变换）"""
        ...
        try:
            ...
            # return iou  # 原始线性IoU返回
            return self.shaped_iou_reward(iou)  # 非线性变换后返回
        except Exception as e:
            print(f"SAM3评估出错: {e}")
            return 0.0
    ...
    def compute_comprehensive_reward(self,
        ...
        # 收集需要LLM评估的think部分（answer改用精确匹配）
        all_prompts = []
        prompt_to_idx = []  # 记录每个 prompt 对应的样本 idx
        answer_rewards = {}  # 存储 answer 的精确匹配结果
        for idx, (content, sol) in enumerate(zip(contents, solution)):
            ...
            # 只为 think 构建 judge prompt
            all_prompts.append(self._build_prompt(gt_think, model_think))
            prompt_to_idx.append(idx)

            # answer 使用精确匹配
            answer_rewards[idx] = 1.0 if model_answer.strip().lower() == gt_answer.strip().lower() else 0.0

        # 执行LLM评估（仅 think）
        think_scores = self._llm_evaluate(all_prompts)
        sample_scores = {idx: {'think': 0.0, 'answer': 0.0} for idx in range(len(contents))}
        for prompt_idx, score in enumerate(think_scores):
            sample_idx = prompt_to_idx[prompt_idx]
            sample_scores[sample_idx]['think'] = score
        for idx, ans_reward in answer_rewards.items():
            sample_scores[idx]['answer'] = ans_reward
        ...
        for idx, (content, sol) in enumerate(zip(contents, solution)):
            ...
            gt_answer = self._extract_tag(sol, 'answer').strip().lower()

            if gt_answer == "normal":
                # 正常样本：没输出description给满分，输出了说明幻觉缺陷给0分
                desc_reward = 0.0 if model_desc else 1.0
            else:
                # 异常样本：只使用SAM3计算IoU作为描述奖励
    ...
    def _format_reward(self, content: str, solution: str) -> float:
        score = 0.0
        gt_answer = self._extract_tag(solution, 'answer').strip().lower()
        if re.search(r'<answer>.*?</answer>', content, re.DOTALL):
            score += 0.3
        if gt_answer == "normal":
            # 正常样本：没有<description>才是正确格式
            if not re.search(r'<description>.*?</description>', content, re.DOTALL):
                score += 0.3
        else:
            # 异常样本：有<description>才是正确格式
