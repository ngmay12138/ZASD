...
class MVTecAnomalyDetector:
    def __init__(self, model_path, device=None, batch_size=4, tensor_parallel_size=1):
        ...
        # 固定的提示词
        # self.prompt = "这张图片中的物体是否存在异常？若存在异常回答yes，否则回答no"
#         self.prompt = """请仔细分析这张图片中的物体，判断是否存在异常。

# ## 任务说明
# 你是一个异常检测专家，需要识别图像中物体的缺陷、损坏、异常状态或不符合标准的情况。
# ## 具体指令
# 1. 首先观察物体的整体状态、结构和表面特征
# 2. 检查是否存在以下异常类型（包括但不限于）：
#    - 物理损伤：裂纹、划痕、凹陷、破损
#    - 形态异常：变形、扭曲、尺寸不符
#    - 颜色/纹理异常：褪色、斑点、污染、不均匀
#    - 功能异常：位置错误、装配不当、缺失部件
#    - 安全风险：泄漏、过热、电气问题表现
# 3. 对比正常状态下的物体特征
# ## 输出格式要求
# 如果不存在异常，请严格按以下格式回答，并将描述和分析替换为你的回答，不要添加任何额外文本：
# <description>"简要描述你观察到的关键特征"</description><analysis>"对观察到的物体为什么是正常的分析"</analysis><answer>"不存在异常"</answer>
# 如果存在异常，请严格按以下格式回答，并将描述、分析以及异常位置替换为你的回答，不要添加任何额外文本：
# <description>"简要描述你观察到的关键特征"</description><analysis>"对观察到的物体为什么是异常的分析"</analysis><location>"异常的具体位置"</location><answer>"存在异常"</answer>
# 现在请分析这张图片："""
...
def main():
    ...
    parser.add_argument("--model-path", type=str, default="my_model/qwen3_vl_2b_lorasft")
    parser.add_argument("--dataset-path", type=str, default="data/dtd")
    parser.add_argument("--meta-file", type=str, default="data/dtd/meta.json")
    parser.add_argument("--batch-size", type=int, default=4,
                        help="批处理大小")
    parser.add_argument("--tensor-parallel-size", type=int, default=2,
                        help="GPU并行数量")
    parser.add_argument("--output-file", type=str, default="result_cot/dtd.json",
                        help="输出结果文件")
    parser.add_argument("--report-file", type=str, default="result_cot/dtd.txt",
                        help="报告文件")
